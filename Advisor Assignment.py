...
for filename in importableCSVs: #can't seem to use this w/ lambda and map
    ...
    SQLTable = advising.SQLTableSubclass()
    SQLTable.Columns = csvObj.Columns
    SQLTable.addRows(csvObj.Data)


advisorSQLDB = advising.SQLTableSubclass()
adv_counts = advisorSQLDB.export('advisor_assignment_count', where_statement='WHERE STDNT_ENRL_STATUS = \'Enrolled\'')
...
advising.CSVTableSubclass(csvData=all_assignments,csvColumns=header_list).export(f'{envDict["unfiltered_file"]}')
header_list.insert(0,"Advisor Suggestion:")
filtered_export = advising.CSVTableSubclass(csvData=all_assignments_filtered,csvColumns=header_list)
