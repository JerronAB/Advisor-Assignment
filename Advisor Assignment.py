        ...
        def IDColumns(row): 
            ...
            for sublist in IDList.Data:
                if sublist[1] == row[4]: 
                    ...
                    break
            if columns == True: return [row[1],row[2],advID,row[4],row[5],row[7],row[8],row[12]] 
            return [row[1],advID]
...
if path.exists(envDict['filtered_file']) and not path.exists(envDict['peoplesoft_file']):
    ...
    finalizeAdvChanges(envDict['filtered_file'],'~/OneDrive - KCTCS/Email Advisors COMMAS temp.csv',columns=True,delimiter_str=';')
